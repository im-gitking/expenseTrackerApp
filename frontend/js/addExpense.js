...
const addToExpenseList = (expense) => {
  ...
  newLi.innerHTML = `<span>${expense.expenseamount} - ${expense.category} - ${expense.description} </span><button class="deleteExpense" id="${expense._id}">✘</button>`;
...
async function postExpenses(e) {
  try {
    ...
    const postedExpense = await axios.post(
...
async function deleteExpense(e) {
  try {
    if (e.target.classList.contains("deleteExpense")) {
      ...
      const deletRequest = await axios.delete(
        `http://localhost:3000/expenses/addExpense/${expenseId}`,
        { headers: { Authorization: token } }
      );
